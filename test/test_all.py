...
from naive_backlink.models import EvidenceRecord, URLContext, LinkDetails
...
def test_score_with_one_strong_signal():
    ...
    evidence = [
        EvidenceRecord(
            id="e-backlink-001",
            kind="rel-me",
            source=URLContext(url="https://pypi.org/project/foo/", context="origin-page"),
            target=URLContext(url="https://mastodon.social/@alice", context="candidate-page"),
...
def test_score_with_weak_signals():
    ...
    """
    evidence = [
        EvidenceRecord(id="e-1", kind="backlink", classification="weak", source=URLContext("","origin-page"), target=URLContext("","candidate-page")),
        EvidenceRecord(id="e-2", kind="backlink", classification="weak", source=URLContext("","origin-page"), target=URLContext("","candidate-page")),
    ]
    score, label = calculate_score(evidence)
    assert 50 <= score < 80
    assert label == "medium"
